...
for page_no in range(1, total_page_no+1):
    ...
    for invalid_link in page_no_data.find_all('a',{"class":"_1fQZEK"}):
        ...
        if original_price is None :
            ...
            original_price = int(re.sub('\D','',original_price.text))

        discount = product_page_data.find('div',{"class":"_3Ay6Sb _31Dcoz"})
        if discount is None :
            continue
        else :
            discount = int(re.sub('\D','',discount.text))
        ...
        CPU, GPU, RAM, HDD, SSD, Battery, Screen_Resolution, Screen_Type = 0,0,0,0,0,0,0,0
        ...
        df.loc[len(df.index)] = [ link, name.text, original_price, discount, price, rating.text, CPU, GPU, RAM, HDD, SSD, Battery, Screen_Resolution, Screen_Type ]
